...
int main (int argc, char *argv[]) {
	//Comprueba que le pasen los parametros
	if(argc != 2){
		printf ("Audio File Path not provided.\n");
		exit(1);
	}
	
	//Copia y muestra la ruta del archivo
	//printf("%s %s\n",argv[1],argv[2]);
	char audio_file_path[100];
	strcpy(audio_file_path,argv[1]);
	printf("Trying to open audio File: %s\n",audio_file_path);
	
	//Se abre el archivo, hasta donde parece no se le añade nada a 
	//la variable audio_info
